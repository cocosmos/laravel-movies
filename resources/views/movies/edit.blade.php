<x-app>
    ...
    <form method="POST" action="{{route('movie.update', $movie->id)}}" enctype="multipart/form-data">
        ...
        <p>
            <label class="text-gray-100" for="length">{{__("Length")}}</label>
            <input type="number" name="length" id="length" value="{{$movie->length}}" max="500" min="20" required
                   class="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-t-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm">
        </p>
        <p>
        ...
        <p>
            <label for="poster" class="form-label inline-block mb-2 text-gray-100">Select a movie poster</label>

            <input type="file" id="poster" name="poster"
                   class="form-control block w-full px-3 py-1.5 text-base font-normal text-gray-700 bg-white bg-clip-padding border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none">

            <img src="{{asset('storage/uploads/posters/'.$movie->poster)}}" alt="{{$movie->title}}" width="100px"
                 height="150px">
        </p>
